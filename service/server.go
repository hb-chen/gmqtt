...
import (
	...
	"github.com/hb-go/micro-mq/auth"
	"github.com/hb-go/micro-mq/sessions"
	"github.com/hb-go/micro-mq/topics"
	"github.com/hb-go/micro-mq/pool"
...
func (this *Server) ListenAndServe(uri string) error {
	...
	// connection goroutine pool
	connPool := pool.New(10240)
...
func (this *Server) handleConnection(c io.Closer) (svc *service, err error) {
	...
	defer func() {
		if err != nil {
			glog.Errorf("connection close with err:%v", err)
			c.Close()
		}
	}()
...
func (this *Server) checkConfiguration() error {
	...
	this.configOnce.Do(func() {
		...
		this.broker = kafka.NewBroker()
		if err = this.broker.Connect(); err != nil {
			return
		}
		...
		this.topicsMgr, err = topics.NewManager(this.TopicsProvider, this.broker, func(p broker.Publication) error {
			return this.subHandler(p)
		})
...
func (this *Server) subHandler(p broker.Publication) error {
	glog.Debug("[sub] received message:", string(p.Message().Body), "header", p.Message().Header)
