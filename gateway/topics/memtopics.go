...
func (this *memTopics) Unsubscribe(topic []byte, sub interface{}) (int, error) {
	this.smu.Lock()
	defer this.smu.Unlock()
...
func (this *snode) sremove(topic []byte, sub interface{}) (int, error) {
	...
	if len(topic) == 0 {
		...
		return 1, fmt.Errorf("memtopics/remove: No topic found for subscriber")
	}
	...
	n, ok := this.snodes[level]
	if !ok {
		return 0, fmt.Errorf("memtopics/remove: No topic found")
	}

	// Remove the subscriber from the next level snode
	nodesNum := len(this.snodes)
	if sNum, err := n.sremove(rem, sub); err != nil {
		return 0, err
	} else {
		nodesNum += sNum
	}
